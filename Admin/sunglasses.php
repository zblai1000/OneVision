...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if (empty($_POST['code'])) {
        ...
        $sql = "SELECT id FROM items WHERE code = ?";
        ...
        if($stmt = mysqli_prepare($OneVision, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){
                    $code_error = "This product code is already taken.";
                    $errors = $errors + 1; 
                } else{
    ...
    // if everything is ok, try to upload file
        ...
        $path = "../sunglasses/". $imageName;
   
        //add to database here
        $q = "INSERT INTO items (itemName, category, code, picture, info, price, promotion) VALUES ('$i', 'sunglasses', '$c', '$path', '$in', '$p', '$pro')";
...
<div class="productDiv" style="width: 90%; text-align: left; margin: 20px;">
    <form action="sunglasses.php" method="post" enctype="multipart/form-data">
    ...
    <br>
        ...
        <input type="submit" value="Upload Item" name="submit" class="btn btn-primary" >
    </form>
</div>

    <h1 style="text-align: center;">Sunglasses</h1>
...
// Define the query:
$q = "SELECT id, itemName, code, price, category, info, promotion, registration_date FROM items WHERE category='sunglasses' ORDER BY $order_by LIMIT $start, $display";		
...
<tr style="background-color: #00cc00;">
	...
	<td align="left"><b><a href="sunglasses.php?sort=iN">Item Name</a></b></td>
    <td align="left"><b><a href="sunglasses.php?sort=cod">Code</a></b></td>
    <td align="left"><b><a href="sunglasses.php?sort=cat">Category</a></b></td>
    <td align="left"><b><a href="sunglasses.php?sort=pri">Price</a></b></td>
    <td align="left"><b>Description</b></td>
    <td align="left"><b><a href="sunglasses.php?sort=pro">Promotion</a></b></td>
    <td align="left"><b><a href="sunglasses.php?sort=rd">Date Registered</a></b></td>
</tr>
';
...
if ($pages > 1) {
	...
	if ($current_page != 1) {
		echo '<a href="sunglasses.php?s=' . ($start - $display) . '&p=' . $pages . '&sort=' . $sort . '">Previous</a> ';
	}
	
	// Make all the numbered pages:
	for ($i = 1; $i <= $pages; $i++) {
		if ($i != $current_page) {
			echo '<a href="sunglasses.php?s=' . (($display * ($i - 1))) . '&p=' . $pages . '&sort=' . $sort . '">' . $i . '</a> ';
		} else {
			echo $i . ' ';
		}
	} // End of FOR loop.
	...
	if ($current_page != $pages) {
		echo '<a href="sunglasses.php?s=' . ($start + $display) . '&p=' . $pages . '&sort=' . $sort . '">Next</a>';
	}
